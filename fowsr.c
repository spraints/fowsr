...
04.03.16 OE8APR new pymultimonaprs format
*/

#define VERSION "V2.0.160304"
...
// Store output file in requested format
    ...
    for(i=0; i<data_count; i++)
    {
                    ...
                    for (j=0;j<WS_MULTIMON_RECORDS;j++) {
                        if (j==WS_MULTIMON_HOURLY_RAIN || j==WS_MULTIMON_DAILY_RAIN) {
                            CWS_decode(&m_buf[multimon_format[j].pos],
                                    multimon_format[j].ws_type,
                                    multimon_format[j].scale,
                                    multimon_format[j].offset,
                                    s2);
                        } else {
                            CWS_decode(&m_buf[current_pos+multimon_format[j].pos],
                                    multimon_format[j].ws_type,
                                    multimon_format[j].scale,
                                    multimon_format[j].offset,
                                    s2);
        ...
        // Just write out the last record to json file
        // If other format is used, always write the record
        if ((arg=='m'&&(current_pos==end_pos))||arg!='m') {
            strcat(s1,"\n");
            fputs(s1,f);
        }
...
/*---------------------------------------------------------------------------*/
void buildJson(char *sWeatherData) {
    char *aWeatherData[10];
    int i = 0;

    aWeatherData[i] = strtok(sWeatherData, ";");
...
int main(int argc, char **argv)
{
    ...
    int mflag = 0; // Create pymultimonaprs.log
    ...
    while ((c = getopt (argc, argv, "bcdf:n:rpswxv:")) != -1)
    {
        switch (c)
        {
            ...
            case '?':
                ...
                printf("Credits to Michael Pendec, Jim Easterbrook, Timo Juhani Lindfors, Andreas Pachler (OE8APR)\n\n");
                printf("See http://www.github.com/spraints/fowsr for more information\n\n");
                printf("options\n");
                printf(" -f[p|s|w|x|f|m] set Logformat for weather data\n");
                ...
                printf(" -fm Logfile in pymultimonaprs format\n");
    ...
    if(0==CWS_Open()) { // Read the cache file and open the weather station
        ...
        if (mflag)
            CWF_Write('m', LogPath, "pymultimonaprs");
        if (pflag)
